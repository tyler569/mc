...
define_packets! {
    mod serverbound {
    enum ServerBoundHandshakePacket : handshake {
        Handshake {
    ...
    enum ServerBoundStatusPacket : status {
        Request {},
        Ping { payload: i64 },
    }

    enum ServerBoundLoginPacket : login {
        LoginStart {
            username: String,
        },
        EncryptionResponse {
    ...
    enum ServerBoundPlayPacket : play {
        TeleportConfirm {
            teleport_id: VarInt,
        },
        QueryBlockNbt {
    ...
    enum ClientBoundHandshakePacket : handshake {}

    enum ClientBoundStatusPacket : status {
        Response { json_data: String },
        Pong { payload: i64 },
    }

    enum ClientBoundLoginPacket : login {
        Disconnect {
            reason: String,
        },
        EncryptionRequest {
    ...
    enum ClientBoundPlayPacket : play {
        SpawnEntity {
            entity_id: VarInt,
            object_uuid: Uuid,
    ...
    fn write(&self, writer: &mut impl std::io::Write) -> anyhow::Result<()> {
        match self {
            Self::Handshake(handshake) => {
                handshake.protocol_version.minecraft_write(writer)?;
                handshake.server_address.minecraft_write(writer)?;
                handshake.server_port.minecraft_write(writer)?;
                handshake.next_state.minecraft_write(writer)?;
            }
        }
...
impl ClientBoundStatusPacket {
    fn read(reader: &mut impl std::io::Read) -> anyhow::Result<Self> {
        let packet_id = VarInt::minecraft_read(reader)?;
